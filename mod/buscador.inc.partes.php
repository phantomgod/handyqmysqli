...
{ 
    ...
    foreach ($aPalabras as $sPalabra) { 
		...
		@$iTotal = mysql_result(mysqli_query($mysqli, "SELECT COUNT(".$aCampos[0].") FROM `partedetrabajo` WHERE 1$preq"), 0); 
...
{ 
    ...
    for ($iTemp = $iInicio; $iTemp < $iFinal; $iTemp++) { 
        echo (($iTemp == $_GET['pag'])?$iTemp+1:'<a href="?seccion=busca_partes&q='.$_GET['q'].'&pag='.$iTemp.'">'.($iTemp+1).'</a>').' '; 
    } 
