...
function onDelete() {
    if ( confirm('<?php echo INSPECTORES_QUIERE_BORRAR; ?>\n <?php echo RECUERDE_SELECCIONAR; ?>')==true) {
        return true;
    } else {
        return false;
    }
...
if ($result) {
    ...
    echo "<form method='post' action='?seccion=borrados_inspectores' OnSubmit='return onDelete(); '>";


        echo "<table class='sortable'>";
            echo "<caption>";
            echo BORRAR_INSPECTOR;
            echo "</caption>";
    ...
    while ($row = $result->fetch_object()) {
                    ...
                    echo "<tr>
                        <td>$id</td>
                        <td>$inspector</td>
                        <td><a href='?seccion=inspectores_admin2c&amp;aktion=change_id&amp;id=$id'><img src=$imgsrc width=80px height=113px></a></td>
                        <td>$activo</td>
