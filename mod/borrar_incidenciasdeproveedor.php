...
function onDelete() {
    if ( confirm('<?php echo INCIDENCIAS_QUIERE_BORRAR; ?>\n <?php echo RECUERDE_SELECCIONAR; ?>')==true) {
        return true;
    } else {
        return false;
    }
...
$query = "SELECT * FROM incidenciasdeproveedor";
...
if ($result) {
      ...
      // create a new form and then put the results into a table.
        ...
        echo "<form method='post' action='?seccion=borradas_incidenciasdeproveedor' OnSubmit='return onDelete(); '>";
        echo "<table class='sortable'>";
            echo "<caption>";
            echo PROVEEDORES_BORRAR_INCIDENCIAS;
            echo "</caption>";
                echo "<tbody>";
                    echo "<tr>";
                    echo "<th>ID</th>";
    ...
    while ($row = $result->fetch_object()) {
                        ...
                        $codigoincidencia = $row->codigoincidencia;
                        $afectaa = $row->afectaa;
                        $fecha = $row->fecha;
