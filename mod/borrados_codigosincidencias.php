<?php
/**
* Mod RECIBE Y BORRA códigos de incidencias
* de proveedor
* 
* PHP version 5
* 
...
if ($_POST['delete']) { // from button name="delete" 
    ...
    for ($i=0;$i<$countCheck;$i++) {
            $del_id  = $checkbox[$i]; 
            $sql = "DELETE from codigosincidencias where id = $del_id";
            $result = $mysqli->query($sql) or die(mysqli_error($mysqli)); 
    }
    if ($result) { 
            echo AINFORMES_INFORME_BORRADO; 
            echo "<br /><br /><a href=?seccion=borrar_codigosincidencias>Borrar otro código</a>"; 
    } else {
           echo "Error:";
