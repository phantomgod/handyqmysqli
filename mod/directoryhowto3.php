...
function Get_Select_tree($mysqli,$root,$nombre,$compare=0)
{
        global $mysqli;
    ...
    if ($root==0) {
		...
		$tnombre = (isset($tnombre)) ? $tnombre : ''; 
		
        echo "<select name='$tnombre'>\n";
...
function testFields(&$errores,&$fields)
{
    ...
    if (isset($_POST["tnombrecontacto"]) && !empty($_POST["tnombrecontacto"])) {
            $fields["nombrecontacto"]=$_POST["tnombrecontacto"];
    } else {
            $fields["nombrecontacto"]="";
            $errores["nombrecontacto"]="<font class=framefont_#9fff30>Debe indicarse una persona de contacto.</font>";
    }
    if (isset($_POST["tnombre"]) && !empty($_POST["tnombre"])) {
            $fields["nombre"]=$_POST["tnombre"];
    } else {
            $fields["nombre"]="";
            $errores["nombre"]="<font class=framefont_#9fff30>La seccion no puede estar vacia.</font>";
    }
...
?>
			...
			<td width="40%" valign="top" style="padding: 10px">&nbsp;Añadir
				...
				<form action="<?php $_SERVER['PHP_SELF']?>" method="post">
					* Titulo del documento:<br> 
						<input type="text" class="form-control input-xxlarge" id="ttitulo" name="ttitulo"	maxlength="255"
							   value=<?php
                if (isset($_POST["ttitulo"]) && !empty($_POST["ttitulo"])) {
                    $fields["titulo"]=$_POST["ttitulo"];
                   echo $_POST["ttitulo"];
			...
			<br />
					* Link (Enlace real):<br>
					<input type="text" class="form-control input-xxlarge" id="tlink" name="tlink"	maxlength="255"					
					       value=<?php
                if (isset($_POST["tlink"]) && !empty($_POST["tlink"])) {
                    $fields["link"]=$_POST["tlink"];
                    echo $_POST["tlink"];
            ...
            ?>
					...
					departamentos:<br> 
					<input type="text" class="form-control input-xlarge" id="tclave1" name="tclave1"
						value=<?php
                if (isset($_POST["tclave1"]) && !empty($_POST["tclave1"])) {
                    $fields["clave1"]=$_POST["tclave1"];
                    echo $_POST["tclave1"];
			...
			?>><br>
					* email de contacto:<br> 
					<input type="text" class="form-control input-xlarge" id="temail" name="temail"
						value=<?php
                if (isset($_POST["temail"]) && !empty($_POST["temail"])) {
                    $fields["email"]=$_POST["temail"];
                    echo $_POST["temail"];
            ...
            ?>><br>
					* persona de contacto:<br>
					<input type="text" class="form-control input-xlarge" id="tnombrecontacto" name="tnombrecontacto"
						value=<?php

                if (isset($_POST["tnombrecontacto"]) && !empty($_POST["tnombrecontacto"])) {
                    $fields["nombrecontacto"]=$_POST["tnombrecontacto"];
                    echo $_POST["tnombrecontacto"];
                }
            ?>><br>
                ...
                if (isset($_POST["tnombre"]) && !empty($_POST["tnombre"])) {
                    $fields["nombre"]=$_POST["tnombre"];
                    $compare=$_POST["tnombre"]>0?$_POST["tnombre"]:0;
                 }
                    $compare="";
                 get_select_tree(0, "nombre", $compare);
