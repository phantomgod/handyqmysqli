...
{ 
    ...
    foreach ($aPalabras as $sPalabra) { 
		...
		@$iTotal = mysql_result(mysqli_query($mysqli, "SELECT COUNT(".$aCampos[0].") FROM `inspecciones` WHERE 1$preq"), 0);
...
{ 
    ...
    if (isset($_GET['pag'])) {
        ...
        for ($iTemp = $iInicio; $iTemp < $iFinal; $iTemp++) { 
            echo (($iTemp == $_GET['pag'])?$iTemp+1:'<a href="?seccion=busca_inspecciones&q='.$_GET['q'].'&pag='.$iTemp.'">'.($iTemp+1).'</a>').' '; 
        } 
