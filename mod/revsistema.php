...
<table class="print">
	<tbody>
		<tr>
			<td><a href="?seccion=revsistema&amp;aktion=add"><?php echo REVISAR_SISTEMA; ?></a>
    ...
    if ((empty($_POST['fecha']))
        ...
        echo '<form action="" method="POST">';
            echo '<h1>'.FECHA.':</h1>';
				...
				echo '<input id="date" class="datepicker" name="fecha" value="' . SELECCIONAR_FECHA . '" />'; // Date picker
        ...
        $sql = mysqli_query($mysqli, "SELECT * FROM objetivosdatosgenerales WHERE Ano >=11 ORDER BY Id DESC");
...
/**
    * Change these to your own credentials*/
    ...
    $result = mysqli_query($mysqli, "SELECT mes, global FROM globalclientes");
    if ($result) {
...
/*------------------------------------------------FIN DEL GRÁFICO DE NO CONFORMIDADES POR ÁREA--------------------------------------*/
    $result = mysqli_query($mysqli, "SELECT area, cantidad FROM ncsporarea");
if ($result) {
...
/*------------------------------------------------GRÁFICO DE INCIDENCIAS DE ALMACÉN--------------------------------------------*/
                        ...
                        /*echo '<iframe width="1100px" height="300px" frameborder="0" scrolling="no" marginheight="0" marginwidth="0" src="mod/incidenciasdealmacen_revsistema.php">';
                        echo '<p>Your browser does not support iframes.</p>';
                        echo '</iframe>'; */
...
?>
	...
	<script type="text/javascript">
        ...
        chart.Set('chart.title', 'Incidencias en inspecciones de almacén');
        chart.Set('chart.colors', ['red','#9fff30','transparent']);
        chart.Set('chart.title.yaxis', 'Nº');
...
/*----------------------------------GRÁFICA DE INCIDENCIAS DE PROVEEDOR----------------------------------*/
                        ...
                        /*echo '<iframe width="1100px" height="300px" frameborder="0" scrolling="yes" marginheight="0" marginwidth="0" src="mod/incidenciasdeproveedor_revsistema.php">';
                        echo '<p>Your browser does not support iframes.</p>';
                        echo '</iframe>'; */
...
if ($aktion == "change") {
    ...
    while ($row = mysqli_fetch_row($sql)) {
            echo "<tr>";
                echo "<td><a href='?seccion=revsistema&amp;aktion=change_id&amp;id=".$row['0']."'>".$row['0']."</a></td>";
                echo "<td><a href='?seccion=revsistema&amp;aktion=change_id&amp;id=".$row['0']."'>".$row['1']."</a></td>";
...
if ($aktion == "change_id") {
    if ((empty($_POST['fecha']))
        ...
        echo '<form action="" method="POST">';
		  ...
		  echo '<input id="date" class="datepicker" name="fecha" value="' . $data[1] . '" />'; // Date picker
