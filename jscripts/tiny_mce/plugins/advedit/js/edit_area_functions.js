    ...
    EditArea.prototype.load= function(){
    var strURL;
    if(this.settings["ajax_type"].length>0){
       strURL = this.settings["ajax_type"] + '?plg=LOAD0';
    }else{
       alert('No ajax, Script!\najax_type: your_script.(PHP ASP CGI)\nPOST method,\nRead manual to customize\nor set save_callback');
       return true;
    }

    this.ucc = window.open(strURL,'load','width=300,height=450,left=70,top=50');
    this.ucc.focus();
    };
    


    EditArea.prototype.save= function(){
