...
{ 
    ...
    foreach ($aPalabras as $sPalabra) { 
        ...
        $iTotal = mysql_result(mysql_query("SELECT COUNT(".$aCampos[0].") FROM `enlaces` WHERE 1$preq"), 0);
        $aResultados=mysql_query("SELECT * FROM `$sTabla` WHERE 1$preq LIMIT ".($_GET['pag']*$iPaginas).", $iPaginas"); 
...
{ 
    ...
    if (isset($_GET['pag'])) {
        ...
        for ($iTemp = $iInicio; $iTemp < $iFinal; $iTemp++) { 
            echo (($iTemp == $_GET['pag'])?$iTemp+1:'<a href="?seccion=busca_enlaces&q='.$_GET['q'].'&pag='.$iTemp.'">'.($iTemp+1).'</a>').' '; 
        } 
...
{ 
    ...
    while ($aResultado = mysql_fetch_array($aResultados)) { 
        ...
        foreach ($aPalabras as $sPalabra) {
            ...
            echo "<br><b><a href=\"".$sUrl.$aResultado[$sId]."\" target='_self'>".substr($aResultado[$sTitulo], 0, 50)."</a></b><br>".$sVistaPrevia."<br><a href=\"".$sUrl.$aResultado[$sId]."\" target='_self'>".substr($sUrl.$aResultado[$sId], 0, 80)."</a><br>"; 
              
              
             // echo "<br><b><a href=\"".$sUrl.$aResultado[$sId]."\" target='_top'>".substr($aResultado[$sTitulo], 0, 50)."</a></b>
               //   <br>".$sVistaPrevia."<br><a href=\"".$sUrl.$aResultado[$sId]."?keepThis=true&TB_iframe=true&height=500&width=800\" title='' class='thickbox'>".substr($sUrl.$aResultado[$sId], 0, 80)."</a><br>"; 
