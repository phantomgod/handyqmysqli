<?php 
/**
* Mod buscar auditorías al sistema
* 
* PHP version 5
* 
* @category Mod
...
{ 
    ...
    if ( ! isset( $_GET['pag'] ) ) {
        ...
        $iTotal = mysql_result(mysql_query("SELECT COUNT(".$aCampos[0].") FROM `extintores` WHERE 1$preq"), 0); 
...
{ 
    ...
    for ($iTemp = $iInicio; $iTemp < $iFinal; $iTemp++) { 
        echo (($iTemp == $_GET['pag'])?$iTemp+1:'<a href="?seccion=busca_extintores&q='.$_GET['q'].'&pag='.$iTemp.'">'.($iTemp+1).'</a>').' '; 
    } 
...
{ 
    ...
    while ($aResultado = mysql_fetch_array($aResultados)) { 
        ...
        foreach ($aPalabras as $sPalabra) {
            ...
            echo "<br><b><a href=\"".$sUrl.$aResultado[$sId]."\">".substr($aResultado[$sTitulo], 0, 50)."</a></b><br>".$sVistaPrevia."<br><a href=\"".$sUrl.$aResultado[$sId]."\">".substr($sUrl.$aResultado[$sId], 0, 80)."...</a><br>"; 
            $it = 1; 
        }
    }
